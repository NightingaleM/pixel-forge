...
void main() {
  ...
  // 1D vertical Gaussian blur on luminance (input from horizontal pass)
  float blurredLum = 0.0;
  for (int i = 0; i < 9; i++) {
    vec2 sampleUv = vUv + vec2(0.0, offsets[i] * (radius / 4.0) * texel.y);
    float lum = texture2D(uImage, sampleUv).r; // grayscale from blur_h pass
