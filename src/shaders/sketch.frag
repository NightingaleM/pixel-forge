...
void main() {
  ...
  // 2. Multi-directional Sobel edge detection at 4 angles
  float totalEdge = 0.0;
  ...
  // 5. Invert: white background, black lines
  float line = 1.0 - edge;
  ...
  if (uHatching > 0.5) {
    ...
    // Primary hatching - horizontal-ish lines
    float hatch1 = sin(pos.x * 1.2 + pos.y * 0.8) * 0.5 + 0.5;
    hatch1 = smoothstep(0.3, 0.7, hatch1);

    // Secondary hatching - steeper angle
    float hatch2 = sin(pos.x * 0.7 - pos.y * 1.3) * 0.5 + 0.5;
    hatch2 = smoothstep(0.35, 0.65, hatch2);

    // Tertiary hatching - vertical lines for darkest areas
    float hatch3 = sin(pos.x * 0.3 + pos.y * 2.0) * 0.5 + 0.5;
