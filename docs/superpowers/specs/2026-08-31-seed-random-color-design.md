...
- **兼容性**：8 个无 color 参数的风格编码序列不变 → 旧种子**完全兼容**；3 个 color 风格（sketch/lightshadow/animelight）序列 +1 档 → 色板特性后生成的种子失效（本就是本次修复对象，无额外损失）。不 bump SEED_VERSION（那会让全部 8 个无辜风格种子陪葬）。
...
- 快照追平：animelight 全 max 码长 16 → 17（+1 color 档）。
